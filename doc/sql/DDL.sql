...
(
    ...
    `type`        TINYINT(2)   NOT NULL DEFAULT 2 COMMENT '菜单类型。0：菜单组，1：父类菜单项，2：普通菜单项',
    `name`        VARCHAR(32)  NOT NULL COMMENT '菜单名',
    `router_path` VARCHAR(100) NULL     DEFAULT NULL COMMENT '前端路由路径',
    `icon`        VARCHAR(50)  NULL     DEFAULT NULL COMMENT '图标名，如info，timeline',
    `item_order`  TINYINT(2)   NOT NULL COMMENT '排序，越大越往后',
    `create_time` DATETIME     NOT NULL DEFAULT CURRENT_TIMESTAMP() COMMENT '创建时间',
    `update_time` DATETIME     NOT NULL DEFAULT CURRENT_TIMESTAMP() ON UPDATE CURRENT_TIMESTAMP() COMMENT '更新时间',
    `deleted`     TINYINT(1)   NOT NULL DEFAULT 0 COMMENT '是否已删除，0未删除，1已删除',
    PRIMARY KEY (`uuid`)
...
(
    ...
    `color`       VARCHAR(20) NOT NULL DEFAULT 'blue lighten-1' COMMENT '色彩css，如blue lighten-1',
    `create_time` DATETIME    NOT NULL DEFAULT CURRENT_TIMESTAMP() COMMENT '创建时间',
    `update_time` DATETIME    NOT NULL DEFAULT CURRENT_TIMESTAMP() ON UPDATE CURRENT_TIMESTAMP() COMMENT '更新时间',
    `deleted`     TINYINT(1)  NOT NULL DEFAULT 0 COMMENT '是否已删除，0未删除，1已删除',
    PRIMARY KEY (`uuid`)
